### 注释版本的 InteractionMeshRetargeter，增加了更详细的注释说明每个部分的功能和逻辑，帮助理解代码结构和算法流程。注释内容包括类和函数的目的、参数说明、关键步骤的解释等，旨在使代码更易读和易于维护。
...
class InteractionMeshRetargeter:
    ...
    def retarget_motion(
        ...
        if q_nominal_list is not None:
            ...
            q_locked_list[0, self.q_a_indices] = q_a_init # q_a_init中，用人的初始位置当机器人位置，用人指向物体的方向当机器人朝向，把所有关节先设成 0。

        q_locked_list[:, -7:] = object_poses_augmented
        # 当augment为true时，q_locked_list的最后7维被设置为object_poses_augmented，在优化过程中，这些维度将被锁定为augment后的物体位姿。
        # 当augment为false时，object_poses_augmented == object_poses。


        q = np.copy(q_locked_list[0])
        retargeted_motions = [q] # retargeted_motions是一个列表，初始时包含了第一帧的配置q。后续每一帧优化得到的新配置都会被追加到这个列表中，最终返回时会转换成numpy数组并去掉第一帧（第一帧是初始配置，不是优化结果）。 
        ...
        with tqdm(range(num_frames)) as pbar:
            for i in pbar:
                ...
                # self.smplh_mapped_joint_indices = [self.demo_joints.index(name) for name in self.laplacian_match_links]
                # self.smplh_mapped_joint_indices 是根据laplacian_match_links中的关节名称在demo_joints中找到对应的索引
                # human_joint_motions[i, self.smplh_mapped_joint_indices] 就是取出当前帧中这些关节的位置信息，作为人类关节在物体坐标系下的点位置，用于后续构建交互网格和计算拉普拉斯坐标。
                ...
                adj_list = get_adjacency_list(source_tetrahedra, len(source_vertices)) # 得到的是一个列表，列表的长度等于顶点的数量，每个元素是一个列表，包含与该顶点相邻（通过tetrahedra连接）的顶点索引。
                target_laplacian = calculate_laplacian_coordinates(source_vertices, adj_list) # 计算demo数据的laplacian坐标，作为优化的目标。这个函数会根据顶点位置和邻接关系计算每个顶点的拉普拉斯坐标，拉普拉斯坐标是一个描述顶点相对于其邻居位置的向量，优化过程中会尝试保持这些拉普拉斯坐标不变，从而保持网格的局部形状。
    ...
    def solve_single_iteration(
        ...
        ) # 如果物体是ground，就在world frame计算雅可比和点位置，否则在object frame计算雅可比和点位置
        robot_link_keys = list(self.laplacian_match_links.keys())
        V_r = len(robot_link_keys) # 参与匹配的机器人关键点个数
        V_o = len(obj_pts_local) # 物体点的个数
        V = V_r + V_o
        ...
        target_lap_vec = target_laplacian.reshape(-1)  # (3V,) 代表目标拉普拉斯向量

        w_v = (self.laplacian_weights * np.ones(V)).astype(float)  # (V,)
        sqrt_w3 = np.sqrt(np.repeat(w_v, 3))

        # Decision variables
        dqa = cp.Variable(len(self.q_a_indices), name="dqa")  ##### 这里的裁剪索引需要注意
        ...
        # Linear equality
        constraints += [cp.Constant(J_L[:, self.q_a_indices]) @ dqa - lap_var == -lap0_vec] # 这里裁剪了第二次，需要后期检查
    ...
    def _prefilter_pairs_with_mj_collision(self, threshold: float):
        ...
        if not hasattr(self, "_saved_margins"): # 判断是否存在_saved_margins属性
    ...
    def _build_transform_qdot_to_qvel_fast(self, use_world_omega=True):
        ...
        for j in range(1, self.robot_model.njnt):
            ...
            if jt in (mujoco.mjtJoint.mjJNT_HINGE, mujoco.mjtJoint.mjJNT_SLIDE):
                ...
                raise NotImplementedError("BALL joint block not implemented.") #当前函数不支持球形关节，如果模型里有球形关节，需要实现它的块
    ...
    def _calc_contact_jacobian_from_point(self, body_idx: int, p_body: np.ndarray, input_world=False):
        ...
        mujoco.mj_jac(self.robot_model, self.robot_data, Jp, Jr, p_W, int(body_idx))  # Jp = J_v，给出的 Jp 是对 qvel 的雅可比
    ...
    def _calc_manipulator_jacobians(
        ...
        mujoco.mj_forward(self.robot_model, self.robot_data) # 让mijoco根据新的qpos计算出最新信息
        ...
        for name, link_name in links.items():
            ...
            if point_offsets is not None:
                pC_B = point_offsets # 要跟踪的点 C 不在 body 原点；这里是点 C 在 body 坐标系 B 下的坐标。
            else:
            ...
            if obj_frame:
                p_XC = obj_rot_inv @ (pos_world - obj_pos) # p_XC表示pos_world在物体坐标系下的坐标，计算方法是先将pos_world平移到以物体为原点的坐标系下（即减去obj_pos），再乘以obj_rot_inv将其旋转到物体坐标系的方向上。
                J_XC = obj_rot_inv @ J # J_XC表示雅可比矩阵J在物体坐标系下的表示。
            else:
                p_XC = pos_world
                J_XC = J
