...
class InteractionMeshRetargeter:
    ...
    def __init__(
        ...
        self.debug = debug
        self._quat_update_total = 0
        self._quat_update_etasp = 0
        self._quat_update_fallback = 0
        ...
        # ETaSP proximal coefficient: tau = dt, so the quadratic coefficient is 1 / (2 * tau).
        self.smooth_weight = 1.0 / (2.0 * (1.0 / 30.0))
    ...
    def retarget_motion(
        ...
        if self._quat_update_total > 0:
            etasp_ratio = 100.0 * self._quat_update_etasp / self._quat_update_total
            fallback_ratio = 100.0 * self._quat_update_fallback / self._quat_update_total
            print(
                "Quaternion update path stats: "
                f"ETASP {self._quat_update_etasp}/{self._quat_update_total} ({etasp_ratio:.1f}%), "
                f"fallback normalization {self._quat_update_fallback}/{self._quat_update_total} "
                f"({fallback_ratio:.1f}%)."
            )
    ...
    def solve_single_iteration(
        ...
        J_lap_active = J_L[:, self.q_a_indices]
        lap_residual = lap0_vec - target_lap_vec
        lap_gradient = 2.0 * (J_lap_active.T @ ((sqrt_w3**2) * lap_residual))
        lap_gradient = np.asarray(lap_gradient, dtype=float).reshape(-1)
        ...
        obj_terms.append(cp.Constant(lap_gradient) @ dqa)
        ...
        if (w_nominal_tracking > 0) and (q_a_nominal is not None):
            ...
            if idx.size > 0:
                nominal_gradient = np.zeros(self.nq_a)
                nominal_gradient[idx] = 2.0 * w_nominal_tracking * (q_a_n_last[idx] - q_a_nominal[idx])
                obj_terms.append(cp.Constant(nominal_gradient) @ dqa)

        # Q_diag cost
        Qd = np.asarray(self.Q_diag, dtype=float).reshape(-1)
        q_gradient = 2.0 * Qd * q_a_n_last
        obj_terms.append(cp.Constant(q_gradient) @ dqa)
        ...
        quat_indices = np.arange(3, 7)
        quat_active = np.isin(self.q_a_indices, quat_indices)
        self._quat_update_total += 1
        if np.array_equal(self.q_a_indices[quat_active], quat_indices):
            self._quat_update_etasp += 1
            q_star[3:7] = self._apply_etasp_quaternion_update(q[3:7], dqa_star[quat_active])
        else:
            self._quat_update_fallback += 1
            q_star[3:7] /= np.linalg.norm(q_star[3:7]) + 1e-12

        return q_star, cost

    @staticmethod
    def _apply_etasp_quaternion_update(q_prev_quat: np.ndarray, delta_quat: np.ndarray) -> np.ndarray:
        r_prev = q_prev_quat.copy()
        r_prev /= np.linalg.norm(r_prev) + 1e-12

        eta = delta_quat - np.dot(r_prev, delta_quat) * r_prev
        eta_norm = np.linalg.norm(eta)
        if eta_norm > 1.0:
            eta /= eta_norm
            eta_norm = 1.0

        r_next = np.sqrt(max(1.0 - eta_norm**2, 0.0)) * r_prev + eta
        r_next /= np.linalg.norm(r_next) + 1e-12
        return r_next
