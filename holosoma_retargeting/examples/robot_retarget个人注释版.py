...
def create_task_constants(
    ...
    if task_type == "robot_only":
        obj_name = task_config.object_name or "ground" # 如果指定了object_name，就用指定的，否则默认是ground。task_config.object_name默认是 None，可以从 CLI 配置里覆盖它。
...
def initialize_robot_pose(
    ...
    if task_type == "object_interaction":
        ...
        q_init = _compute_q_init_base(task_type, data_format, human_joints, object_poses, constants) # 初始化机器人姿态，得到q_init
        # Convert object_poses to MuJoCo order
        object_poses = convert_object_poses_to_mujoco_order(object_poses)
        object_poses_augmented = convert_object_poses_to_mujoco_order(object_poses_augmented)
...
def main(cfg: RetargetingConfig) -> None:
    ...
    """
    如果没有指定 data_format，则根据 task_type 使用默认值：
    "robot_only": "smplh",
    "object_interaction": "smplh",
    "climbing": "mocap",
    """
    ...
    if cfg.motion_data_config.robot_type != robot or cfg.motion_data_config.data_format != data_format:
        cfg.motion_data_config = MotionDataConfig(data_format=data_format, robot_type=robot) # 确定data_format 和 robot_type 在 motion_data_config 中正确设置
    ...
    # 当 task_type 是 robot_only 时，object_local_pts 和 object_local_pts_demo 是相同的地面点。
    # 当 task_type 是 object_interaction 时，object_local_pts 和 object_local_pts_demo 都是物体表面点，
    # object_local_pts 是原始物体点，object_local_pts_demo 是按 smpl_scale 缩放后的 demo/source 侧物体点。
